...
public class RopeScript : MonoBehaviour {
	...
	*/
	...
	public void BuildRope()
	{
		...
		segments = (int)(Vector3.Distance(transform.position,target.position)*resolution) - 1;
		...
		TieToPlayer(segments);

		// Attach the joints to the target object and parent it to this object	
		HingeJoint end = target.gameObject.AddComponent<HingeJoint>();
	...
	void TieToPlayer(int n)
	{
		endJoint = GameObject.FindGameObjectWithTag("Player").AddComponent<HingeJoint>();
		if(endJoint != null && lastSegment != null){
			endJoint.connectedBody = lastSegment.GetComponent<Rigidbody> ();
		}
	}
	...
	void AddJointPhysics(int n)
	{
		...
		HingeJoint newJoint = joints [n].AddComponent<HingeJoint> ();
		//newJoint.spring.spring = 10;
...
//		ph.swing1Limit = limit_setter;
		...
		if(n==1){		
			newJoint.connectedBody = transform.rigidbody;
		} else
		{
			newJoint.connectedBody = joints[n-1].rigidbody;	
		}
		lastSegment = joints [n];
	...
	public void DestroyRope()
	{
		...
		{
//			Destroy(joints[dj].GetComponent<HingeJoint>());
//			Destroy(joints[dj].GetComponent<SphereCollider>());
//			Destroy(joints[dj].GetComponent<Rigidbody>());
			Destroy(joints[dj]);
			//			Destroy();	
		}
